...
pub fn rm_outliers(helptabx: &Vec<f64>, helptaby: &Vec<f64>, xs: &Vec<f64>, ys: &Vec<f64>)
    -> (Vec<f64>, Vec<f64>)
{
    ...
    for i in 0..helptaby.len()
    {
        ...
        {
           if helptaby[i] <= meanf+sigmaf && helptaby[i] >= meanf-sigmaf {
               xsh.push(helptabx[i]); ysh.push(helptaby[i]); }
        }
    }
...
pub fn read_data(path: &str, normback: bool, noback: bool) -> (Vec<f64>,Vec<f64>,Vec<f64>,f64,f64,
                                                               ...
                                                               Vec<f64>)
{
    ...
    datavec = datavec.into_iter().filter(|&i| i.trim() != "" && i.chars().next().
        unwrap() != '#').collect::<Vec<_>>();
    let (mut time, mut flux, mut err): (Vec<f64>, Vec<f64>, Vec<f64>) = (vec![0f64;datavec.len()],
                                                                         vec![0f64;datavec.len()],
                                                                         vec![0f64;datavec.len()]);
    ...
    {   
        ...
        {
            if iii >= 0 && iii < istart as i32 {help_t1.push(time[iii as usize]);
                help_f1.push(flux[iii as usize]);}
            else if iii < time.len() as i32 && iii > istop as i32 {help_t2.push(
                time[iii as usize]); help_f2.push(flux[iii as usize]);}
        }
        iii += 1;
    }
    ...
    let (mut time1, mut flux1, mut err1): (Vec<f64>,Vec<f64>,Vec<f64>) = (vec![0f64;istop-istart+1],
                                                                          vec![0f64;istop-istart+1],
                                                                         vec![0f64;istop-istart+1]);
